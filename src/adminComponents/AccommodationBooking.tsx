...
const AccommodationBooking = ({ providerId }: { providerId: string }) => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
        setLoading(true)
      try {
        const res = await fetch(`/api/booked-accomo?providerId=${providerId}`);
        ...
        <div key={booking.id} className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl font-sans">
          <div className="md:flex">
            <div className="p-6">
              ...
              <p className="text-xl font-medium text-primary dark:text-primary2">Accommodation Type: <span className="text-xl text-main font-normal dark:text-main2 font-sans">{booking.accommodation?.type}</span></p>
              <p className="text-xl font-medium text-primary dark:text-primary2">Accommodation Id: <span className="text-xl text-main font-normal dark:text-main2 font-sans">{booking.accommodation?.id}</span></p>
              <p className="text-xl font-medium text-primary dark:text-primary2">Accommodation Price: <span className="text-xl text-main font-normal dark:text-main2 font-sans">{booking.accommodation?.price}</span></p>
