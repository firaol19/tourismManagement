...
const DestinationBooking = ({ providerId }: { providerId: string }) => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
        setLoading(true)
      try {
        const res = await fetch(`/api/booked-destination?providerId=${providerId}`);
  ...
  return (
    <div className="space-y-6 mb-5">
      {bookings.map((booking) => (
        <div key={booking.id}>
          {booking.status !== "allRefund" && (
            <div  className="max-w-md mx-auto bg-white rounded-xl shadow-md overflow-hidden md:max-w-2xl font-sans">
            <div className="md:flex">
              <div className="p-6">
                ...
                <p className="text-xl font-medium text-primary dark:text-primary2">Destination: <span className="text-xl text-main font-normal dark:text-main2 font-sans">{booking.destination?.name}</span></p>
                <p className="text-xl font-medium text-primary dark:text-primary2">Destination Id: <span className="text-xl text-main font-normal dark:text-main2 font-sans">{booking.destination?.id}</span></p>
                <p className="text-xl font-medium text-primary dark:text-primary2">Interance Price: <span className="text-xl text-main font-normal dark:text-main2 font-sans">{booking.destination?.interance}</span></p>
